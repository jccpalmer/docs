...
Here, I will show you how to set up Pi-hole for network-wide ad blocking. These instructions also include optional steps for setting up Unbound for recursive DNS resolution in case you don't want to use a third-party DNS resolver like Google or Cloudflare.
...
- A machine that can run 24/7
- 2GB of storage (4GB is preferred)
- 512MB of RAM
- Access to your router's DNS settings

In order to use Pi-hole, it is recommended that you have a supported Linux distribution (distro) running. The Pi-hole project officially supports Raspberry Pi OS, Ubuntu, Debian, Fedora, and CentOS. You can also install Pi-hole in a Docker container, which is not described here.
...
You'll need to follow the below steps to get Pi-hole installed.
